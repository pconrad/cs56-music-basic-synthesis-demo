...
public class ADSREnvelopedContinuousSound extends EnvelopedSound{
    ...
    */
    public ADSREnvelopedContinuousSound(	
			      ...
			      int sampleRate,
			      SourceDataLine l){
	super(frequency,attackTime+decayTime+sustainTime+releaseTime,sampleRate,l);
    ...
    public String toString(){
        return super.toString()+" Amplitude: "+amplitude+" Sustain Amplitude: "+sustainAmplitude+" Atack Time: "+attackTime+" Decay Time: "+decayTime+ " Sustain Time: "+sustainTime+" Release Time: "+releaseTime;
    }
    protected double generateAmplitude(double time, double amplitude){
	double amp = amplitude;
	if(time<this.attackTime){
	    double deltaAmp=this.amplitude/(getSampleRate()*this.attackTime);      //the rate of growth of amplitude per sample
